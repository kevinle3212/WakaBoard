...
> **Archived 2026-08-29.** This is the acceptance ledger for the completed
> production-readiness audit, kept verbatim as the historical record. It lived at
> the repository root as `GATES.md`; the root file now holds the ledger for the
> current multi-platform and design work. Twenty-two of its twenty-three gates were
> met and G22 was honestly abandoned — the on-device and human sensory verification
> only Kevin can unblock, which now lives in `TODO.md`. The `OWNS:` line below is
> inert.
...
- [x] G14: Apple submission requirements are satisfied — a `PrivacyInfo.xcprivacy` for
      the app and the widget extension, both wired into the generated project, the
      export-compliance key in both `Info.plist`s, and a complete app icon: every size
      the catalog declares present at its declared dimensions, alpha on the macOS
      entries and none on the iOS marketing icon, named by both app targets.
  CHECK: node scripts/audit-checks.mjs apple
  EXPECT: APPLE_OK
  EVIDENCE: exit 0 — 2026-08-28. Caught that XcodeGen regenerates the plists, so the key had to move into `project.yml`. The icon half was proved by regression rather than assertion: in a scratch copy of the tree, deleting a declared PNG, adding alpha to the iOS icon, unwiring `ASSETCATALOG_COMPILER_APPICON_NAME`, and resizing a macOS entry each failed the gate, and the real tree still returns `APPLE_OK`. Note for anyone reading the built bundle: `actool` writes only the 16pt and 128pt pairs into the legacy `AppIcon.icns`, which is not a missing size — `assetutil --info` on the compiled `Assets.car` shows all ten macOS representations plus the iOS icon, and that is what `CFBundleIconName` resolves against on macOS 11 and later.
